			...
			<view class='coupon-list' :style="{'margin-top':!orderShow?'0':'50rpx'}">
				<block v-if="coupon.list.length">
					...
					<view class='item acea-row row-center-wrapper' v-for="(item,index) in coupon.list"
						...
						<view class='money acea-row row-column row-center-wrapper' :class="[isUsableClassName(item,'main_bg','moneyGray')]">
							<view v-if="item.way === 1">￥<text class='num'>{{item.money?Number(item.money):''}}</text></view>
							<view v-if="item.way === 2"><text class='num'>{{discountFilter(item.discount)}}</text></view>
							<view class="pic-num">满{{item.minPrice}}元可用</view>
						</view>
						<view class='text'>
							<view class='condition line2'>
								<span class='line-title' :class="[isUsableClassName(item,'select','gray')]" v-if='item.useType===1'>通用</span>
								<span class='line-title' :class="[isUsableClassName(item,'select','gray')]"
									v-else-if='item.useType===3'>品类</span>
								<span class='line-title' :class="[isUsableClassName(item,'select','gray')]" v-else>商品</span>
								<span>{{item.name}}</span>
							</view>
							<view class='data acea-row row-between-wrapper'>
								...
								<view class='' v-if="item.isUsable !== undefined && !item.isUsable">不符合条件</view>
								<template v-else>
	...
	export default {
		...
		methods: {
			...
			getCouponUser: function(index, id) {
				...
				const isUsable = list[index].isUsable
				console.log(isUsable !== undefined && !isUsable)
				if (list[index].isUse == true && this.openType == 0 || (isUsable !== undefined && !isUsable) ) return true;
			...
			discountFilter(val) {				
				return val !== null ? val * 10 + '折' : ''
			},
			//优惠券是否可用显示的样式
			isUsableClassName(item,onClassName,offClassName){			
				const isUseCheck = item.isUse ? offClassName : onClassName
				if(typeof item.isUsable === 'boolean') return item.isUsable ? isUseCheck :	offClassName	
				return isUseCheck
			}
 		}
	}
