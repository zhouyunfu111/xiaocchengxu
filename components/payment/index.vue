<template>
	<view>
		<view class="payment" :class="pay_close ? 'on' : ''">
			...
			<view class="item acea-row row-between-wrapper" @click='$u.debounce(goPay.bind(this,item.number || 0 , item.value), 500)'
...
<script>
	...
	export default {
		...
		computed: mapGetters(['systemPlatform','userInfo','productType','openId']),
		created(){
			this.payConfig();
		...
		watch:{
			...
			totalPrice() { 
				const {yuePayStatus,payWeixinOpen } = this.payConfigObj
				const { nowMoney } = this.userInfo
				if(+payWeixinOpen === 1 && +yuePayStatus === 1 && +nowMoney !== 0) {
					const residuePrice = (Math.round((+this.totalPrice - +nowMoney) *100) / 100).toFixed(2)
					this.payMode[2].payStatus = +this.totalPrice > +nowMoney ? 1 : 0;
					this.payMode[2].title = `可用余额:${nowMoney}(余额：￥${nowMoney}+微信支付：￥${residuePrice})`
				}				
				this.payMode[1].payStatus = +yuePayStatus === 1 && +this.totalPrice <= +nowMoney ? 1 : 0;		
			},
		},
		methods: {
			close: function() {
			...
			payConfig(){
				getPayConfig().then(res=>{
					const { yuePayStatus,payWeixinOpen } = res.data
					this.payConfigObj = res.data
					this.payMode[0].payStatus = +payWeixinOpen === 1 ? 1 : 0;							
					this.payMode[1].payStatus = +yuePayStatus === 1 ? 1 : 0;
				})
			},
			...
			weixinPay(jsConfig){
				let that = this;
				console.log('weixinPay',jsConfig)
				// #ifdef MP
