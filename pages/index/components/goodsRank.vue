	...
	<view :data-theme="theme">
		<view class='hotList' v-if="productList.length > 2">
			<view class='title acea-row row-between-wrapper'>
				...
				<view class='more' hover-class="none" v-if="isMore" @click="more()">更多
				...
				<block>
					<view class="item acea-row row-middle" :class="{'lei' : index < 2}" v-for="(item,index) in productList.slice(0,3)" :key="index" @click="toDetail(item.id)">
						...
						<view class="ml_11 flex-column justify-between flex-1">
							...
							<view class="price flex justify-between" v-if="styleName === 'price'">
								<view>
							...
							<view class="sales flex justify-between flex-1" v-if="styleName === 'sales'">
								<view class="sales-box" >
									<view class="">
										<view class="item-title">大盘销量</view>
										<view class="item-num">{{item.marketSales}}</view>
									</view>
									<view class="">
										<view class="item-title">本店进货量</view>
										<view class="item-num">{{item.quantityIn}}</view>
									</view>
									<view class="">
										<view class="item-title">同售店铺</view>
										<view class="item-num">{{item.sameSaleNum}}</view>
									</view>
								</view>			
								<view class="cart_icon">
									<text class="iconfont icon-gouwuche7"></text>
		...
		methods:{
			...
			more(){
				...
				uni.navigateTo({
					animationType: animationType.type,
					animationDuration: animationType.duration,
					url:`/pages/activity/promotionList/index?type=${typeInfo.type}&name=${typeInfo.name}`
				})
			}
		}
	}
