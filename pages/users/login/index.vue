<template>
	<div class="login-wrapper" :data-theme="theme">
		<view class="login-header">
			...
			<u--text text="如果手机号未注册，将自动创建账号" color="#999"></u--text>
		</view>
		<div class="whiteBg" v-if="formItem === 1">
			<view class="form-box">
				<input type="number" class="texts" placeholder="输入手机号码" v-model="account" maxlength="11"/>
				<button class="btn-code" :style="{opacity: isPhone ? '1' : '0.5'}" @click="getCode">获取短信验证码</button>
			</view>
...
<script>
	...
	export default {
		...
		computed:{
			isPhone(){
				const reg = /^1(3|4|5|7|8|9|6)\d{9}$/i
		...
		methods: {
			getCode() { 
				...
				if(!this.isPhone) return uni.showToast({title:'请填写完整的手机号码', icon: 'none'})
				uni.navigateTo({
					url: `/pages/users/get_code/index?account=${this.account}&authKey=${this.authKey}`
				})
			},
			// 苹果登录
			appleLogin() {
