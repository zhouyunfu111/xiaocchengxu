<template>
	<view :data-theme="theme">
				...
				<view class='item borRadius14' v-for="(item,index) in addressList" :key="index">
					...
					<view class='operation acea-row row-between-wrapper'>
						...
						<view class='acea-row row-middle'>
							<view @click='editAddress(item.shopId)'><text class='iconfont icon-bianji'></text>编辑</view>
		...
		<!-- <view class='footer acea-row row-between-wrapper' style="justify-content: center;"> -->
			<!-- #ifdef APP-PLUS -->
			<view class='addressBnt bg_color on' @click='addAddress'><text
					class='iconfont icon-tianjiadizhi'></text>添加新地址</view>
			<!-- #endif -->
			<!-- #ifdef MP-->
			<!-- <view class='addressBnt bg_color' @click='addAddress' style="width: 700rpx;"><text class='iconfont icon-tianjiadizhi'></text>添加新地址
			</view> -->
		...
		methods: {
			...
			radioChange: function(e) {
				...
				setAddressDefault(address.id).then(res => {
					...
					that.$store.commit('SET_SHOP_NAME', address.shopName)
					uni.$emit('updateCate')
					uni.$emit('upDateHome')
					that.$util.Tips({
						title: '设置成功',
			...
			editAddress: function(shopId) {
				...
				uni.navigateTo({
					url: `/pages/users/user_shop/index?id=${shopId}`
				})
			},
			/**
			 * 删除地址
