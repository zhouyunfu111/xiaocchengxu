	...
	<view :data-theme="theme">
		<view class='order-details'>
			...
			<view v-if="isGoodsReturn==false" class="pad30">
				...
				<orderGoods
					:orderInfo="orderInfo"
					:evaluate='evaluate' :productType="orderInfo.type" :orderId="order_id" :ids="id" :uniId="uniId" :cartInfo="cartInfo"
			...
			<view class="pad30">
				...
				<view class='wrapper borRadius14'>
					...
					<view class='item acea-row row-between' v-if="isGoodsReturn">
						<view>售后编号：</view>
						<view class='conter acea-row row-middle row-right'><text class="text-overflow">{{orderInfo.refundOrderNo}}</text>
							<!-- #ifndef H5 -->
							<text class='copy' @tap='copy'>复制</text>
							<!-- #endif -->
							<!-- #ifdef H5 -->
				...
				<view v-if="isGoodsReturn" class='wrapper borRadius14' >
					...
					<view class='item acea-row row-between' v-if="orderInfo.refundType !== null">
						<view>售后类型：</view>
						<view class='conter'>{{afterSaleTypeList[orderInfo.refundType]}}</view>
					</view>
				...
				<view class='wrapper borRadius14'>
					...
					<view class='actualPay acea-row row-right'>
						{{ isGoodsReturn ? '退款金额' : '实付款'}}：<text class='money'>￥{{ isGoodsReturn ?  orderInfo.refundPrice : orderInfo.payPrice }}
						</text>
					</view>
				</view>
				...
				<view class='footer acea-row row-right row-middle' v-if="isGoodsReturn==false">
					...
					<navigator hover-class="none" 
						:url="'/pages/goods/goods_return/index?orderId='+orderInfo.orderId"
						class='bnt cancel' 
						v-else-if="orderInfo.outboundStatus === 0 && !!orderInfo.paid && orderInfo.refundStatus === 0 && orderInfo.type!==1">
						申请退款
					</navigator>
...
<script>
	...
	export default {
		...
		computed:{
			...
			isRefund(){
				return this.orderInfo.paid === true && this.orderInfo.refundStatus === 0 && this.orderInfo.type!==1
			}
		},
		onLoad: function(options) {
			...
			this.isReturen = options.isReturen === "1"
			this.isGoodsReturn = options.isReturen === "1"
		...
		onShow() {
			if (this.isLogin) {
				...
				this.getOrderRefundType() //获取售后类型数据字典列表
			} else {
				toLogin();
			}
		},
		...
		methods: {
			...
			async getOrderInfo() {
				...
				try {
					const res = this.isReturen ? await orderRefundDetail(that.order_id) : await getOrderDetail(that.order_id)
					uni.hideLoading();
					...
					// that.isGoodsReturn = [2].includes(res.data.refundStatus);
			...
			async getOrderRefundType(){
				try {
					...
					this.afterSaleTypeList = Object.keys(data).map(item => (data[item]))
					console.log(this.afterSaleTypeList)
				} catch(e) {
					console.log(e)
				}				
			}
