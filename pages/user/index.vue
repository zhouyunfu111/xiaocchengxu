						...
						<view class="user-card">
							<view class="user-info">
								...
								<image class="avatar" :src='userInfo.avatar' v-if="userInfo.avatar && uid"
									@click="goEdit()"></image>
								<image v-else class="avatar" :src="urlDomain+'crmebimage/perset/staticImg/f.png'" mode="" @click="goEdit()"></image>
								<view class="info">
					...
					<view class="contenBox" id="pageIndex">
						...
						<view class="user-menus" style="margin-top: 20rpx;" @click.native="bindEdit('userMenus')">
							...
							<view class="list-box">
								...
								<button class="item" hover-class='none' @click="onClickService" v-if="chatConfig.telephone_service_switch === '1'">
									<image :src="servicePic"></image>
									<text>联系客服</text>
								</button>
...
<script>
	...
	export default {
		...
		computed: mapGetters(['isLogin', 'chatUrl', 'uid','bottomNavigationIsCustom','theme']),
		data() {
			return {
		...
		onLoad() {
			app.globalData.theme = this.theme
		...
		methods: {
			...
			getTokenIsExist() {
				tokenIsExistApi().then(res => {
					...
					if (this.isLogin && tokenIsExist) {
						...
						this.userInfo = {}
						this.userMenu = {}
						this.orderMenu = this.orderMenu.map(item => ({...item,num: 0}))
					}
				})
			},
			bindEdit(name) {
			...
			getMyMenus: function() {
				...
				getMenuList().then(res => {
					const {data:{routine_my_menus,routine_my_banner}} = res
					const MyMenus = routine_my_menus.filter(item => {
						const isPromoter = (item.url =='/pages/promoter/user_spread_user/index' && !that.userInfo.isPromoter)
						const isCourier = (item.url =='/pages/users/photoUpload/index' && !that.userInfo.isCourier)
						return !(item.url =='/pages/service/index' || isPromoter || isCourier)
					})
					that.$set(that, 'MyMenus', MyMenus);
					
					that.wechatUrl = routine_my_menus.filter((item) => {
						return item.url.indexOf('service') !== -1
					})
					routine_my_menus.map((item) => {
						if (item.url.indexOf('service') !== -1) that.servicePic = item.pic
					})
					// that.imgUrls = res.data.routine_my_banner
					if(routine_my_banner){
						that.imgUrls = routine_my_banner
					}
				});
			},
			// 编辑页面
