		...
		<view class="product-con skeleton" :style="{visibility: showSkeleton ? 'hidden' : 'visible'}">
				...
				<view class='navbarH' :style='{height:navH + "rpx"}'>
					<view class='navbarCon acea-row' :style="{ paddingRight: navbarRight + 'px' }">
						...
						<view class="select_nav flex justify-center align-center" id="home"
							:style="{ top: homeTop + 'rpx' }" :class="returnShow?'lang':'circle'">
							<text v-if="returnShow" class="iconfont icon-fanhui2 px-20" @tap="returns"></text>
							<navigator url='/pages/index/index' open-type='switchTab' class="iconfont icon-shouye7 px-20"></navigator>
							...
							<view class='wrapper mb30 borRadius14'>
								<view class='share acea-row row-between row-bottom'>
									<view class='x-money skeleton-rect flex align-baseline'>￥
										<text class='num font-44'>
											{{attr.productSelect.priceAfterCoupon ? attr.productSelect.priceAfterCoupon : attr.productSelect.price || '****' }}
										</text>
										<text>{{attr.productSelect.unitName ? `/${attr.productSelect.unitName}` : ''}}</text>
										<view class="fjs-m-l-10" v-if="attr.productSelect.isSuit">
											{{ `(仅￥${attr.productSelect.unitSalePriceAfter}/${attr.productSelect.productUnitName})` }}
										</view>
										<view class="practical-text fjs-m-l-10" v-if="attr.productSelect.priceAfterCoupon">劵后价</view>
								...
								<view class="slider-banner banner">
									<swiper indicator-dots="true" :autoplay="autoplay" :circular="circular"
										...
										<swiper-item v-for="(item,indexw) in good_list" :key="indexw">
											<view class="list acea-row" :id="'list'+indexw">
												<view class="item" v-for="(val,indexn) in item.list" :key="indexn"
													...
													<view class="money theme_price flex align-baseline">
														¥{{displayPriceBasedOnConditions(val)}}
														<text>{{val.unitName ? `/${val.unitName}` : ''}}</text>
														<view class="practical-text" v-if="val.priceAfterCoupon">劵后价</view>
													</view>
													<view class="unit-price" v-if="val.suitPrice">
														{{ `(仅￥${val.suitUnitPrice}/${val.suitUnitName})` }}
													</view>
												</view>
											</view>
										</swiper-item>
			...
			<view class='footer acea-row row-between-wrapper'>
				...
				<block v-if="type === 'normal'">
					...
					<view class="acea-row skeleton-rect" :class=" !attr.productSelect.price ?  'noPrice' : 'bnt' " v-if="attr.productSelect.stock <= 0">
						<form @submit="$u.debounce(joinCart,500)" report-submit="true">
							<button class="joinCart bnts" form-type="submit">加入购物车</button>
						</form>
						<form report-submit="true"><button class="bnts bg-color-hui no-stock-btn" form-type="submit">已售罄</button>
						</form>
					</view>
					<view class="acea-row skeleton-rect" :class="!attr.productSelect.price ?  'noPrice' : 'bnt' "   v-else>
						<form @submit="$u.debounce(joinCart,500)" report-submit="true"><button class="joinCart bnts"
		...
		methods: {
			...
			ChangeAttr: function(res) {
				...
				if (productSelect) {
					...
					this.$set(this.attr.productSelect, "salePrice", null)
					this.$set(this.attr.productSelect, "isSuit", null)
			...
			DefaultSelect: function() {
				...
				if (productSelect && productAttr.length) {
					...
					this.$set(this.attr.productSelect, 'unitName', this.productInfo.unitName);
					this.$set(this.attr.productSelect, 'daysQuota', this.productInfo.daysQuota);
					this.$set(this.attr.productSelect, "daysQuotaFixed", this.productInfo.daysQuotaFixed);
					this.$set(this.attr.productSelect, 'priceAfterCoupon', this.productInfo.priceAfterCoupon);
					this.$set(this.attr.productSelect, "unitPriceAfterCoupon", this.productInfo.unitPriceAfterCoupon)
					this.$set(this.attr.productSelect, "productUnitName", this.productInfo.productUnitName)
					this.$set(this.attr.productSelect, "residuePromoNum", this.productInfo.residuePromoNum)
					this.$set(this.attr.productSelect, "daysPromoPrice", +this.productInfo.daysPromoPrice)
					this.$set(this.attr.productSelect, "salePrice", +this.productInfo.salePrice)
					this.$set(this.attr.productSelect, "isSuit", this.productInfo.isSuit)
					this.$set(this, "attrValue", "");
					this.$set(this, "attrTxt", "请选择");
				} else if (!productSelect && !productAttr.length) {
					this.$set(this.attr.productSelect, "storeName", this.productInfo.storeName);
			...
			goBuy: Debounce(function(e) {
				const { price, cart_num, daysQuota } =  this.attr.productSelect
				if(!price) return
				if(daysQuota !== null && cart_num > daysQuota) {
					return this.$u.toast('购买数量不能超过限购数量,')
				}
				if (this.isLogin === false) {
					toLogin();
				} else {
			...
			changeSwitch(e) {
				...
				if (productSelect) {
					...
					this.$set(this.attr.productSelect, 'unitName', productSelect.unitName);
...
<style scoped lang="scss">
	...
	.product-con .footer .bnt,.noPrice {
		width: 444rpx;
		height: 76rpx;
	}
	.noPrice .bnts {
