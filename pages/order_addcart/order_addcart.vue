<template>
	<view :data-theme="theme">
		...
		<view 
			...
			:style="{top: navH+'rpx', paddingBottom: bottomNavigationIsCustom ? '175rpx' : 0}" 
								...
								<view class='item acea-row row-between-wrapper'>
									...
									<navigator :url='"/pages/goods/goods_details/index?id="+item.productId' hover-class='none'
										...
										<view class='text'>
											...
											<view class='info line1' v-if="item.suk">属性：{{item.suk}}</view>
											<view class='info line1 flex' 
												@click.stop="showDaypRomoTips(0)" 
												v-if="item.daysPromoPrice && item.residuePromoNum > 0"
											>
												{{`前${item.residuePromoNum + item.unitName}享特价￥${item.daysPromoPrice}/${item.unitName}`}}
												<u-icon name="question-circle" color="#FE5C2D" size="14"></u-icon>
											</view>
											<view class="info line1 flex" 
												@click.stop="showDaypRomoTips(item.isCoupon == '1' ? 2 : 1)" 
												v-if="item.priceType === 1"
											>
												门店特价商品
												<u-icon name="question-circle" color="#FE5C2D" size="14"></u-icon>
											</view>
											<view class='money mt-28' v-if="item.attrStatus">
												￥{{item.twicePrice}}
												<text>{{item.unitName ? `/${item.unitName}` : ''}}</text>
												<view class="practical-text" v-if="item.isCoupon === '1'">劵后价</view>
											</view>
										...
										<view class="carnum-tips" v-show="item.cartNumTips">
											{{item.stockMsg?item.stockMsg:''}}
										</view>
									</navigator>
								</view>
							</block>
		...
		<view class='footer acea-row row-between-wrapper' v-if="cartList.valid.length > 0" :class="bottomNavigationIsCustom?'bottom-custom':''">
			...
			<view class='money acea-row row-middle' v-if="footerswitch==true">
				<text class="estimate-text" v-show="selectValue.length > 0">预估金额</text>
				<text class='price-color' v-if="!!selectCountPrice"> ￥{{selectCountPrice}}</text>
		...
		<u-modal :show="isCartNumTipsShow" title="提示" content='商品已被抢空，请重新选择商品' @confirm="modalConfirm"></u-modal>
		<u-modal
	...
	export default {
		...
		data() {
			return {
				...
				isAllSelect: false, //全选  `库存不足，仅剩${item.stock}包`
				selectValue: [], //选中的数据
		...
		computed: mapGetters(['isLogin','bottomNavigationIsCustom','theme']),
		...
		methods: {
			...
			getTokenIsExist() {
				tokenIsExistApi().then(res => {
					...
					this.canShow = true;
					console.log(this.tokenIsExist,this.isLogin)
					if (this.isLogin && this.tokenIsExist) {
						this.getIndex();
					}
				})
			...
			async getPreOrder() {
				...
				const {cartId,msg,num} = await this.$Order.getPreOrder("shoppingCart", shoppingCartId);
				const index = this.cartList.valid.findIndex(item => item.id === +cartId)
				this.cartList.valid[index].cartNumTips = true
				this.cartList.valid[index].stock = num
				this.$u.toast(msg)
			},
			...
			setAllSelectValue: function(status) {
				...
				const newValid = valid.map(item => {
					...
					return { ...item,checked: item.attrStatus };
				});
			...
			switchSelect: function() {
				...
				if (selectValue.length < 1) {
					...
					for (let index in validList) {
						const prod = validList[index]
						const price = prod.suitPrice ? prod.suitPrice : prod.twicePrice
						if (that.inArray(validList[index].id, selectValue)) {
							selectCountPrice = that.$util.$h.Add(selectCountPrice, that.$util.$h.Mul(validList[index]
								.cartNum, validList[index].vipPrice ? validList[index].vipPrice : price))
						}
					}
					that.selectCountPrice = selectCountPrice;
				}
			...
			subCart: Debounce(function(index) {
				...
				item.cartNumTips = item.cartNum > item.stock
				...
				if (false == status) {
					that.setCartNum(item.id, item.cartNum, function({number,price,msg,out,isCoupon}) {
						that.cartList.valid[index] = item;
						//如果剩余库存为0，那么提示用户"商品已被抢空，请重新选择商品"，需要用户确认，确认后，自动将购物车商品置为失效
						if(number === 0) {
							this.isCartNumTipsShow = true;						
							const invalidItem = this.cartList.valid.splice(index,1);
							this.cartList.invalid.push(...invalidItem);
						}
						if(out === '-1'){
							console.log(">>>>>>商品库存不足")
							that.$u.toast(msg);
							item.stockMsg = msg;
							that.cartList.valid[index].cartNumTips = true
						}
						item.isCoupon = isCoupon;
						item.cartNum = number
						price !== '-1' && (item.twicePrice = price)
						that.switchSelect();
						that.getCartNum();
					});
				}
			...
			addCart: Debounce(function(index) {
				let that = this;
				let item = that.cartList.valid[index];
				// if(+item.cartNum + 1 > item.daysQuota) {
				// 	return this.$util.showToast('已超过限购数量');
				// }			
				if (item.cartNum < item.stock) {
					item.numAdd = false;
					item.numSub = false;
					that.setCartNum(item.id, +item.cartNum + 1, function({number,msg,out,isCoupon,price}) {
						msg && (that.$u.toast(msg))
						that.cartList.valid[index] = item;
						//如果剩余库存为0，那么提示用户"商品已被抢空，请重新选择商品"，需要用户确认，确认后，自动将购物车商品置为失效
						if(number === 0) {
							that.isCartNumTipsShow = true;						
							const invalidItem = that.cartList.valid.splice(index,1);
							that.cartList.invalid.push(...invalidItem);
						}
						//如果剩余库存不为0，那么toast提示用户“商品库存不足，仅剩XX包”，并将库存不足的购物车商品数量下面加上提示：库存不足
						if(out === '-1'){
							that.$u.toast(msg)
							that.cartList.valid[index].cartNumTips = true
						}else{
				...
				getCartCounts(true, 'total').then(async c => {
					...
					for (let i = 0; i < Math.ceil(that.cartCount / that.limit); i++) {
						...
						if (validList.length > 0) {
							for (let index in validList) {
								const {stock = 0, cartNum, id,attrStatus,vipPrice,priceAfterCoupon,price} = validList[index]								
								validList[index].numSub = cartNum === 1;					
								validList[index].numAdd = cartNum === stock;		
								validList[index].twicePrice = vipPrice ? vipPrice : (priceAfterCoupon ? priceAfterCoupon : price || '****');
								//判断购物车的购买数量是否大于stock库存数量，大于就提示
								validList[index].cartNumTips = (attrStatus && cartNum > stock);								
										
								validList[index].stockMsg = (attrStatus && cartNum > stock) ? '商品库存不足，仅剩'+stock+'包' : ''
																						
								validList[index].checked = attrStatus ? (selectValue.push(id) !== 0) : false;
...
<style scoped lang="scss">
	...
	.shoppingCart .list .item .picTxt .text .info {
		font-size: 24rpx;
		color: #999999;
		margin-top: 16rpx;
	}
