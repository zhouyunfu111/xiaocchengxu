...
export function applyRefund(data) {
	return request.get(`order/apply/refund`,data);
}
 ...
 * 获取订单配送时间
 */
export function orderPreHint() {
	return request.get("order/pre/hint");
}

/**
 * 获取满赠满购商品列表
 * @param preOrderNo 预下单编号
 **/ 
export function orderFullList(preOrderNo) {
	return request.get(`order/full/list?preOrderNo=${preOrderNo}`);
}
 ...
 * 
 **/
export function orderRefundType() {
	return request.get(`order/refund/type`)
}
 ...
 * 
 */
export function orderRefundDetail(refundNo) {
	return request.get(`order/refund/${refundNo}`)
}
